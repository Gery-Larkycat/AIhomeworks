...
用法 / Usage:
    ...
    python -m Q3.ablation --experiments baseline,no_bn  # 指定实验
    python -m Q3.ablation --ignore-search       # 忽略搜索结果，用默认参数
"""
...
from Q3.config import TrainConfig  # noqa: E402
from Q3.data import get_cifar100_datasets  # noqa: E402
...
def _train_resnet_with_extractor(config, train_ds, test_ds):
    """
    包装 train_resnet 以传递 save_feature_extractor=True。
    Q3 CIFAR-100 训练需要保存特征提取器供后续迁移学习使用。
    """
    return train_resnet(
...
def main() -> None:
    ...
    output_dir = (
        ...
        if args.output_dir
        else Path(f"outputs/Q3/ablation/{timestamp}")
    )

    print("=" * 60)
    print("ResNet-18 CIFAR-100 Ablation Study")
    print("ResNet-18 CIFAR-100 消融实验")
    print("=" * 60)
    ...
    results = run_ablation_suite(
        ...
        train_fn=_train_resnet_with_extractor,
        get_datasets_fn=get_cifar100_datasets,
