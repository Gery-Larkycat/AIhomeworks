...
用法 / Usage:
    ...
    python -m Q2.ablation --experiments baseline,no_bn  # 指定实验
    python -m Q2.ablation --ignore-search       # 忽略搜索结果，用默认参数
"""
...
from Q2.data import get_cifar10_datasets  # noqa: E402
...
def main() -> None:
    ...
    # Consistent with main.py: auto-load best search params
    base_config = Q2TrainConfig()
    if not args.ignore_search:
        best_params = load_q2_best_params()
    ...
    output_dir = (
        ...
        if args.output_dir
        else Path(f"outputs/Q2/ablation/{timestamp}")
    )

    print("=" * 60)
    print("ResNet-18 CIFAR-10 Ablation Study")
    print("ResNet-18 CIFAR-10 消融实验")
    print("=" * 60)
