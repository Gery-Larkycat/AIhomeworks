...
ResNet-18 CIFAR-10 训练和评估的主入口。

Orchestrates: data loading → hyperparameter search (optional) → training → evaluation → visualization.
编排：数据加载 → 超参数搜索（可选）→ 训练 → 评估 → 可视化。

Uses skorch NeuralNetClassifier for training, shared with Q3.
使用 skorch NeuralNetClassifier 训练，与 Q3 共用管线。
...
# Also add project root for src.Q2 style imports
# 同时添加项目根目录以支持 src.Q2 风格的导入
_project_root = str(Path(__file__).resolve().parents[2])
if _project_root not in sys.path:
    sys.path.insert(0, _project_root)

from Q2.config import Q2TrainConfig  # noqa: E402
from Q2.data import get_cifar10_datasets, get_cifar10_loaders  # noqa: E402
...
def build_config(
    ...
    return Q2TrainConfig(**overrides)
